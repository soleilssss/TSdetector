...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, targets, previous_image = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                images  = images.cuda(local_rank)
                targets = [ann.cuda(local_rank) for ann in targets]
                previous_image  = previous_image.cuda(local_rank)
        #----------------------#
        #   清零梯度
        #----------------------#
        optimizer.zero_grad()
        if not fp16:
            ...
            #   前向传播
            #----------------------#
            outputs         = model_train(images,previous_image)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            #   前向传播
            #----------------------#
            outputs         = model_train_eval(images,previous_image)
