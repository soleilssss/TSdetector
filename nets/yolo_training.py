...
class YOLOLoss(nn.Module):    
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        output[..., :2]     = (output[..., :2] + grid.type_as(output)) * stride  #(bbox_xy + 偏移)*stride，xy投影到输入图像
        output[..., 2:4]    = torch.exp(output[..., 2:4]) * stride  #exp(bbox_wh)*stride, wh投影到输入图像
        return output, grid  ##output:[batchsize,6400,1+4+1], grid:[1,6400,2]
...
#    num_fg_img: 标签分配完成后，总共存在的候选框个数(matrix_matching每列保证一个候选框)
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts):
        # get_assignments函数包括两个步骤， (1) 正样本anchor的初步筛选，(2) 利用SimOTA进行anchor的精细化筛选。
        #-------------------------------------------------------#
        ...
        #  第二步利用SimOTA进行正样本锚点框的精细化筛选的标签分配方法
        #-------------------------------------------------------#
        #   fg_mask                 [n_anchors_all]
        ...
        #-------------------------------------------------------#
        #  Loss函数计算用于标签分配
        pair_wise_ious      = self.bboxes_iou(gt_bboxes_per_image, bboxes_preds_per_image, False) # 所有真实框与候选框之间的IoU
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt, center_radius = 2.5):
        ...
        #-------------------------------------------------------#
        is_in_boxes     = bbox_deltas.min(dim=-1).values > 0.0 # bbox_deltas size:[num_gt,8400,4]
        is_in_boxes_all = is_in_boxes.sum(dim=0) > 0  # bbox_deltas size:[num_gt,8400,4]
        ...
        #-------------------------------------------------------#
        is_in_centers       = center_deltas.min(dim=-1).values > 0.0 # 计算以真实框中心点向外扩大5倍边长，左上角与右下角的坐标
        is_in_centers_all   = is_in_centers.sum(dim=0) > 0   # 计算以真实框中心点向外扩大5倍边长，左上角与右下角的坐标
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        #-------------------------------------------------------#
        #  第一步：某幅图像真实框，与通过初筛获取的预测框计算IoU，然后通过计算的IoU找出最大top10的数据，
        # 尺寸大小为[num_gt, 10]。再由最大top10的数据统计这幅图像每个目标分配的候选框，通过找出cost最小位置分配某个候选框
        ...
        topk_ious, _            = torch.topk(pair_wise_ious, n_candidate_k, dim=1) # 通过topk_ious动态选择框
        ...
        #   num_fg为正样本的特征点个数
        #------------------------------------------------------------#
        fg_mask_inboxes = matching_matrix.sum(0) > 0.0  # 设置同列多个1的列,最小cost为1,其余为0
